...
def rand_basis(ws):
    return {k: torch.randn(size=v.shape, device=v.device) for k, v in ws.items()}


def create_bases(model, kws=None):
    kws = [] if kws is None else kws
    ws0 = copy.deepcopy(model.state_dict())
    bases = [rand_basis(ws0) for _ in range(2)]  # Use two bases
